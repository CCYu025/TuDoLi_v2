...
* **幽靈容器 (Ghost Container / Drag-to-Create)**：
    * 捨棄繁瑣的鍵盤輸入，底部新增「虛線幽靈區塊」。
    * 直接將任務拖入虛線區，系統即刻自動完成「轉職 (Evolve)」並建立新里程碑。
* **五芒星等級制 (Star Rank System)**：
    * 去文字化設計，以「星星數量」代表專案進化深度。
    * 第一階段為 ⭐，第二階段為 ⭐⭐，以此類推，視覺化展現成長成就感。
* **容器解散邏輯 (Hard Delete & Merge)**：
    * 修正舊版「降級」導致的資料混亂。
    * 現在點擊解散鈕會「徹底銷毀容器」並將內部任務安全回歸至 GENESIS 始祖列表。
* **雙軌演進邏輯**：
...
### 1. 幽靈節點顯示問題 (Evolution Node Bug)
* **描述**：在地圖中建立新的「里程碑 (Milestone)」時，該容器在資料庫中標記為 `Evolution Node`。目前主畫面清單尚未加入類型過濾，導致該節點會以「任務」形式錯誤顯示在 GENESIS 清單中。
* **狀態**：待修復。計畫在下個版本於 `app.js` 加入渲染過濾器。
